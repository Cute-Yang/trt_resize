...
template <typename scalar_t>
__device__ __forceinline__ static void get_cubic_upsample_coefficients(scalar_t coeffs[4], scalar_t t) {
    scalar_t A = -0.75;
...
template <typename scalar_t>
__device__ __forceinline__ static scalar_t upsample_get_value_bounded(const scalar_t *data, int batch, int channel,
                                                                      int batchsize, int channels, int height, int width, int y,
                                                                      int x) {
...
template <typename scalar_t>
__device__ __forceinline__ scalar_t area_pixel_compute_source_index(scalar_t scale, int64_t dst_index, bool align_corners,
                                                                    bool cubic) {
    if (align_corners) {
...
template <typename scalar_t>
__global__ void resize_cubic_kernel_torch(const int num_elements, const scalar_t *src, const int batchsize, const int channels,
                                          int srcWidth, int srcHeight, scalar_t *dst, int dstWidth, int dstHeight,
                                          bool align_corners, float height_scale, float width_scale) {
    if (blockIdx.x == 0 && threadIdx.x == 0) {
        printf("src size(%d %d) dst_size(%d %d)\n", srcWidth, srcHeight, dstWidth, dstHeight);
        // printf("the output size is equal to input size,just copy...");
    }
    ...
    CUDA_1D_KERNEL_LOOP(index, num_elements) {
        ...
        if (srcHeight == dstHeight && srcWidth == dstWidth) {
            ...
            for (int n = 0; n < batchsize; n++) {
                for (int c = 0; c < channels; c++) {
                    const scalar_t val =
                        src[n * channels * dstHeight * dstWidth + c * dstHeight * dstWidth + output_y * dstWidth + output_x];
        ...
        scalar_t real_x = area_pixel_compute_source_index(width_scale, output_x, align_corners, /*cubic=*/true);
        int in_x = floorf(real_x);
        scalar_t t_x = real_x - in_x;

        scalar_t real_y = area_pixel_compute_source_index(height_scale, output_y, align_corners, /*cubic=*/true);
        ...
        for (int n = 0; n < batchsize; n++) {
            for (int c = 0; c < channels; c++) {
                ...
                dst[n * channels * dstHeight * dstWidth + c * dstHeight * dstWidth + output_y * dstWidth + output_x] =
                    scalar_t(cubic_interp1d(coefficients[0], coefficients[1], coefficients[2], coefficients[3], t_y));
            }
        }
    }
}
...
template <typename scalar_t>
void resizeGPU(const scalar_t *pIn_d, scalar_t *pOut_d, int batch, int channels, int srcWidth, int srcHeight, int dstWidth,
               int dstHeight, bool align_corners, cudaStream_t stream) {
    ...
    resize_cubic_kernel_torch<<<GET_BLOCKS(n), THREADS_PER_BLOCK, 0, stream>>>(dstWidth * dstHeight, pIn_d, batch, channels,
                                                                               srcWidth, srcHeight, pOut_d, dstWidth, dstHeight,
                                                                               align_corners, height_scale, width_scale);
}

template <typename scalar_t>
void bicubic_interpolate(const scalar_t *input, scalar_t *output, int batch, int channels, int in_height, int in_width,
...
template void bicubic_interpolate<float>(const float *input, float *output, int batch, int channels, int in_height,
                                         int in_width, int out_height, int out_width, bool align_corners, cudaStream_t stream);
