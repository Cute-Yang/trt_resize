...
get_cubic_upsample_coefficients(scalar_t coeffs[4], scalar_t t) {
  scalar_t A = -0.75;
...
template <typename scalar_t>
__device__ __forceinline__ static scalar_t
...
template <typename scalar_t>
__device__ __forceinline__ static scalar_t
upsample_get_value_bounded(const scalar_t *data, int batch, int channel,
...
template <typename scalar_t>
__global__ void
resize_cubic_kernel_torch(const int num_elements, const scalar_t *src,
                          const int batchsize, const int channels, int srcWidth,
                          int srcHeight, scalar_t *dst, int dstWidth,
                          int dstHeight, bool align_corners, float height_scale,
                          float width_scale) {
    ...
    if (srcHeight == dstHeight && srcWidth == dstWidth) {
      ...
      for (int n = 0; n < batchsize; n++) {
        for (int c = 0; c < channels; c++) {
          const scalar_t val =
              src[n * channels * dstHeight * dstWidth +
                  c * dstHeight * dstWidth + output_y * dstWidth + output_x];
