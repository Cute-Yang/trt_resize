...
struct Serializer<
    ...
    static void serialize(void **buffer, T const &value) {
        ...
        // the address need to plus the offset!
        //  *buffer += sizeof(T);
    ...
    static void deserialize(void const **buffer, size_t *buffer_size, T *value) {
        ...
        reinterpret_cast<char const *&>(*buffer) += sizeof(T); // capture the lref!
        *buffer_size -= sizeof(T);
    }

    // static void serialize(char** buffer,T const& value) {
    //     std::memcpy(*buffer,&value,sizeof(T));
    //     *buffer += sizeof(T);
    // }
};
...
template <typename T>
inline size_t serailized_size(T const &value) {
    return Serializer<T>::serialize_size(value);
}
